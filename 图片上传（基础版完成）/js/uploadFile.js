...
//jquery
var $, Promise, layer, uploadLoading;
/**
 * 文件上传类
 */
var UploadFile = /** @class */ (function () {
     ...
     */
    UploadFile.prototype.handleUpload = function (callBack) {
            ...
            return __generator(this, function (_c) {
                switch (_c.label) {
                    case 0:
                        ...
                        if (!UploadFile.files.length) return [3 /*break*/, 5];
                        if (!UploadFile.config.url)
                            return [2 /*return*/, (layer.msg('文件上传路径有误，请检查是否未填写路径'))];
                        if (this.runStatus)
                            return [2 /*return*/, (layer.msg('请勿重复提交，正在上传中...'))];
                        //loading层
                        ...
                        _b = callBack;
                        return [4 /*yield*/, this._handleSendUpload()];
                    case 1:
                        _a = _b.apply(void 0, [_c.sent()]);
                        return [3 /*break*/, 4];
                    case 2: return [4 /*yield*/, this._handleSendUpload()];
                    case 3:
                        _a = _c.sent();
                        _c.label = 4;
                    case 4:
     ...
     * 发送请求
     */
    UploadFile.prototype._handleSendUpload = function () {
        var _this = this;
        //改变状态
        this.runStatus = true;
        //实例化FormData
        ...
        //文件参数
        UploadFile.files.forEach(function (val, key) {
            data.append(UploadFile.config.param + '[' + key + ']', val);
        });
        //文件参数
        //参数
        var param = UploadFile.config.data;
        //判断是否有携带参数
    ...
    /* 元素绑定事件 */
    UploadFile.prototype._handle = function () {
        var _this = this;
        //图片预览
    ...
    //拖拽上传
    UploadFile.prototype._dragUpload = function () {
        var _this = this;
        //保存定时器
        var timer = '';
        //保存拖拽元素
     ...
     */
    UploadFile.prototype._handlePreview = function (index) {
        //保存图集数据
        var data = {
            "title": "",
            "id": 0,
     ...
     */
    UploadFile.prototype._handleDelFile = function (index) {
        index = Number(index);
        //删除文件源
    ...
    /* 引入依赖文件 */
    UploadFile.prototype._includeFile = function () {
        if (!$ && typeof $ != 'function') {
            this.isAction = false;
            return;
        }
     ...
     * @param file 二进制文件
     */
    UploadFile.prototype._handleAsync = function (file) {
        return __awaiter(this, void 0, void 0, function () {
            var res, style, index_1, el_1, reader, delImg_1, preview_1;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, this._checkFile(file)
                        //保存文件
                    ];
                    case 1:
                        res = _a.sent();
     ...
     * @param file 二进制文件
     */
    UploadFile.prototype._checkFile = function (file) {
        //文件类型
        var type = file.type;
        //判断图片是否重复
        var isRepeat = false;
        ...
        //验证图片类型
        var check = UploadFile.config.suffix.length ? UploadFile.config.suffix.filter(function (val) { return file.name.toLowerCase().indexOf(val) != '-1'; }) : [true];
        if (check.length <= 0) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            //判断图片是否重复
            return new Promise(function (resolve, reject) {
                UploadFile.baseFiles.forEach(function (val) {
                    if (!isRepeat) {
                        if (val.src == baseFile)
                            isRepeat = true;
                    }
